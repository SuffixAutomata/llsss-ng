...
public:
  ...
  [[nodiscard]] bool accepts(std::uint16_t neighborhood) const noexcept { return values_[neighborhood & 0x3ffU] != 0; }

  [[nodiscard]] bool accepts_partial(std::uint16_t known, std::uint16_t value) const noexcept {
    std::size_t key = 0;
    std::size_t place = 1;
  ...
  [[nodiscard]] std::size_t storage_bytes() const noexcept { return sizeof(values_) + partial_values_.capacity() * sizeof(std::uint64_t); }
...
private:
  ...
  static const std::array<std::pair<int, int>, 9>& neighborhood_order() {
    static const std::array<std::pair<int, int>, 9> order = {
        std::pair{-1, -1}, std::pair{0, -1}, std::pair{1, -1}, std::pair{-1, 0}, std::pair{0, 0},
  ...
  static bool bit(std::uint16_t value, int index) { return ((value >> index) & 1U) != 0; }

  static void set_bit(std::uint16_t& value, int index, bool state) {
    const auto mask = static_cast<std::uint16_t>(1U << index);
    value = state ? static_cast<std::uint16_t>(value | mask) : static_cast<std::uint16_t>(value & ~mask);
  }
  ...
  static NeighborhoodType make_moore_type() {
    ...
    type.representatives = {
        {'0', 'x', 0b000000000}, {'1', 'c', 0b100000000}, {'1', 'e', 0b010000000}, {'2', 'c', 0b101000000}, {'2', 'e', 0b010100000}, {'2', 'k', 0b010000001},
        {'2', 'a', 0b110000000}, {'2', 'i', 0b010000010}, {'2', 'n', 0b100000001}, {'3', 'c', 0b101000001}, {'3', 'e', 0b010101000}, {'3', 'k', 0b010100001},
        {'3', 'a', 0b110100000}, {'3', 'i', 0b100100100}, {'3', 'n', 0b101100000}, {'3', 'y', 0b101000010}, {'3', 'q', 0b100100001}, {'3', 'j', 0b001001010},
        {'3', 'r', 0b011000010}, {'4', 'c', 0b101000101}, {'4', 'e', 0b010101010}, {'4', 'k', 0b011100001}, {'4', 'a', 0b100100110}, {'4', 'i', 0b101101000},
        {'4', 'n', 0b100100101}, {'4', 'y', 0b101000110}, {'4', 'q', 0b110100001}, {'4', 'j', 0b001101010}, {'4', 'r', 0b011001010}, {'4', 't', 0b111000010},
        {'4', 'w', 0b100100011}, {'4', 'z', 0b110000011}, {'5', 'c', 0b010101110}, {'5', 'e', 0b101000111}, {'5', 'k', 0b101001110}, {'5', 'a', 0b001001111},
        {'5', 'i', 0b011001011}, {'5', 'n', 0b010001111}, {'5', 'y', 0b010101101}, {'5', 'q', 0b011001110}, {'5', 'j', 0b110100101}, {'5', 'r', 0b100101101},
        {'6', 'c', 0b010101111}, {'6', 'e', 0b101001111}, {'6', 'k', 0b101101110}, {'6', 'a', 0b001101111}, {'6', 'i', 0b101101101}, {'6', 'n', 0b011101110},
        {'7', 'c', 0b011101111}, {'7', 'e', 0b101101111}, {'8', 'x', 0b111101111},
    };
  ...
  void parse_hensel(const std::string& rule) {
    ...
    for(std::size_t i = 0; i < rule.size();) {
      ...
      if(ch == '!' || ch == '?') {
        ...
        selected = ch == '!' ? std::pair{false, false} : std::pair{true, true};
        ++i;
        continue;
      }
      if(ch >= '0' && ch <= '8') {
    ...
    for(std::uint16_t bits10 = 0; bits10 < 1024; ++bits10) {
      ...
      values_[bits10] = static_cast<std::uint8_t>(future_center ? found->second.second : found->second.first);
    }
  }
};
